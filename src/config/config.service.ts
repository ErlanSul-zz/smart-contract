...
export class ConfigService {
  ...
  get contractAddress(): string {
    return this.envConfig.CONTRACT_ADDRESS;
  }

  get walletKey(): string {
    return this.envConfig.WALLET_KEY;
  }

  get infuraId(): string {
    return this.envConfig.INFURA_ID;
  }

  get infuraSecret(): string {
    return this.envConfig.INFURA_SECRET;
  }
  private validateInput(envConfig: EnvConfigRaw): ConfigInterface {
    const envVarsSchema: Joi.ObjectSchema = Joi.object({
      ...
      INFURA_SECRET: Joi.string().required(),
    });
